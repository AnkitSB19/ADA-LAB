...
void knapsack(int n, int p[], int w[], int W) {
    ...
    while (cur_w > 0) {
        ...
        // Find the object with the maximum profit-to-weight ratio
        for (i = 0; i < n; ++i) {
            if (used[i] == 0 &&
        ...
        if (maxi == -1) {
            break; // No more objects to consider
        }

        used[maxi] = 1;

        // If the object can fit completely in the remaining space
        if (w[maxi] <= cur_w) {
            cur_w -= w[maxi];
            tot_v += p[maxi];
            ...
            // If only a fraction of the object can fit
            int taken = cur_w;
            cur_w = 0;
